...
export default function Teleprompter() {
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      if (scrollRef.current) scrollRef.current.scrollTop += speed * delta;
  ...
  if (loading) {
    return <div className="fixed inset-0 z-50 flex items-center justify-center bg-background"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  }
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      <div className="border-t bg-card px-4 py-3 safe-bottom">
        <div className="mx-auto flex max-w-md items-center gap-3">
          ...
          <Button size="icon" className={`h-12 w-12 rounded-full ${playing ? "bg-destructive" : "gradient-primary border-0"}`} onClick={() => setPlaying(!playing)}>
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setFontSize(Math.max(16, fontSize - 2))}><Minus className="h-3 w-3" /></Button>
            <span className="text-xs w-6 text-center">{fontSize}</span>
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setFontSize(Math.min(48, fontSize + 2))}><Plus className="h-3 w-3" /></Button>
          </div>
