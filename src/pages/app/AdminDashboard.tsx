...
import { Users, FileText, TrendingUp, Activity, Shield, Loader2, BarChart3, RefreshCw, Crown } from "lucide-react";
...
interface AdminStats {
  ...
  recentUsers: { id: string; email: string; createdAt: string; lastSignIn: string | null; subscriptionStatus?: string }[];
  scriptsByDay: Record<string, number>;
  subscriptions?: { free: number; basic: number; pro: number; enterprise: number };
  mrr?: number;
}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card className="border-primary/30 bg-primary/5">
          <CardContent className="p-4">
            ...
            <p className="mt-2 text-2xl font-bold">R$ {(stats.mrr || 0).toLocaleString('pt-BR')}</p>
            <div className="text-[10px] text-muted-foreground flex gap-2 mt-1">
              <span>Basic: {stats.subscriptions?.basic || 0}</span>
              <span>Pro: {stats.subscriptions?.pro || 0}</span>
              <span>Ent: {stats.subscriptions?.enterprise || 0}</span>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="p-4 pt-0">
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span>Enterprise (R$ 127)</span>
                <span className="font-bold">{stats.subscriptions?.enterprise || 0}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span>Pro (R$ 67)</span>
                <span className="font-bold">{stats.subscriptions?.pro || 0}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span>Basic (R$ 37)</span>
                <span className="font-bold">{stats.subscriptions?.basic || 0}</span>
              </div>
              <div className="flex items-center justify-between text-xs border-t pt-2">
                <span>Grátis</span>
                <span className="font-bold text-muted-foreground">{stats.subscriptions?.free || 0}</span>
              </div>
            </div>
          </CardContent>
        </Card>
                  ...
                  <div key={u.id} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 truncate flex-1 mr-2">
                      ...
                      {u.subscriptionStatus && u.subscriptionStatus !== 'free' && (
                        <span className="bg-primary/20 text-primary text-[8px] font-bold px-1 rounded uppercase">
